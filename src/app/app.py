...
def run_rag(prompt: str, files_list: list):
    ...
    if files_list:
        ...
        print(
            f"Successfully loaded {len(loaded_docs)} document chunks from user files."
        )
...
with gr.Blocks(title="RAG Newspaper article generator") as demo:
    ...
    gr.Markdown(
        "Upload one or more files and enter your prompt for getting a newspaper article below:"
    )
    ...
    with gr.Row():
        ...
        prompt_input = gr.Textbox(
            label="Your Prompt", placeholder="Give me the topic for the article..."
        )
